exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Venice's scale param must be between 1 and 4; enhance must be true when scale is 1.
    const scale = Number(requestBody.scale) || 2;
    if (scale < 1 || scale > 4) {
      return {
        statusCode: 400,
        headers: { 'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'scale must be between 1 and 4.' })
      };
    }
    ...
    const venicePayload = {
      ...
      scale,
      enhance: scale === 1 ? true : !!requestBody.enhance
    };
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        response = await fetch('https://api.venice.ai/api/v1/image/upscale', {
          method: 'POST',
          headers: {
    ...
    console.error('Error proxying upscale request:', error);
    return {
      statusCode: 500,
      headers: {
        'Access-Control-Allow-Origin': '*',
